...
class User {
	...
	private function _info() {
	    $query = "SELECT * FROM user LEFT JOIN fbtoken ON user.fb_id = '". $this->_id ."' AND user.fb_id = fbtoken.fb_id WHERE user.fb_id = '". $this->_id ."'";
		return $this->_sql->query($query);
	}

	public function graph_call($call) {
		$url = 'https://graph.facebook.com/'. $this->_id .'?'. $call .'&access_token='. $this->info['token'];
