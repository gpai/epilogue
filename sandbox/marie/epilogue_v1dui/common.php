...
//  - sampleInsert() = INSERT
//  - sampleChange() = UPDATE
//  - InsertFacebookFriends() = Runs on log-in to display a list of FbFriends for the user to invite
//  - GetFacebookFriends()

$current_database = 'Vixen_firstdb';
...
function UpdateDeceased(){
	...
	$link = mysql_connect('mysql2.speedypuppy.net:3306', 'Vixen_VixGrace', 'cutie');
	$update_this = "UPDATE $current_database.`user_friend_list` SET deceased =  '1' WHERE facebook_user_id = '$deceased_facebook_user_id' ";
	//test to see connection of database
	if (!$link) {
		die('Could not connect: ' . mysql_error());
	}
